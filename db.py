...
from models import DatosEtiqueta, RegistroAuditoriaSync, RegistroPesaje, ResumenDia
...
class PesajeDatabase:
    ...
    def registrar_auditoria_sync(
        ...
        *,
        ...
        """Registra un intento de subida a la API (éxito o error)."""
    ...
    def auditoria_sync(
        ...
        *,
        ...
        """Historial de subidas. desde/hasta en 'YYYY-MM-DD' o datetime."""
        ...
        return [self._row_to_auditoria(r) for r in rows]

    def contar_auditoria_sync(self) -> tuple[int, int]:
        """Retorna (total, exitosos)."""
        with self._lock:
            with self._connect() as conn:
                row = conn.execute(
                    """
